...
def _plot_nature_style(
    ...
    stem = f"Fig_GDE_sectional_formal_{place}"
    fig.savefig(out_dir / f"{stem}.svg", bbox_inches="tight")
    fig.savefig(out_dir / f"{stem}.png", dpi=300, bbox_inches="tight")
    fig.savefig(out_dir / f"{stem}.pdf", dpi=300, bbox_inches="tight")
    plt.close(fig)
    print(f"[GDE] Saved sectional formal figure to figures/{stem}.png")
...
def main() -> None:
    cfg = default_config()
    print("[GDE Formal] Loading data and aggregating to Hour_Min diurnal cycle...")
    ...
    # Use full diurnal cycle (1440 mins)
    ...
    if np.isfinite(r2_growth):
        ...
        print("[GDE] Growth vs inputs regression could not be fitted.")
