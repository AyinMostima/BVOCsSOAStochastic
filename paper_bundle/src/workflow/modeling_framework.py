...
def save_table(df: pd.DataFrame, stem: str, sheet: str) -> None:
    ...
    csv_path = Path("tables") / f"{stem}.csv"
    xlsx_path = Path("tables") / f"{stem}.xlsx"
...
def persist_cache_outputs(
    ...
    cache_dir: Path | str = Path("intermediate"),
) -> None:
...
def load_cached_results(
    cache_dir: Path | str = Path("intermediate"),
    tables_dir: Path | str = Path("tables"),
...
def plot_category_i(df: pd.DataFrame, outputs: Dict[str, pd.DataFrame]) -> None:
    ...
    fig.suptitle("Category I: Linear models for SOA")
...
def run_full_pipeline() -> None:
    from src.workflow import plotting
